...
class SynthesisModule(nn.Module):
    ...
    def __init__(
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            ...
            if res == self.init_res:
                ...
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.const"
                ] = f"{res}x{res}/Const/const"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.epilogue.bias"
                ] = f"{res}x{res}/Const/bias"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.epilogue.style_mod.dense.fc.weight"
                ] = f"{res}x{res}/Const/StyleMod/weight"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.epilogue.style_mod.dense.wscale.bias"
                ] = f"{res}x{res}/Const/StyleMod/bias"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.epilogue.apply_noise.weight"
                ] = f"{res}x{res}/Const/Noise/weight"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.epilogue.apply_noise.noise"
                ] = f"noise{2 * block_idx}"
            else:
                ...
                if fused_scale:
                    ...
                    self.pth_to_tf_var_mapping[
                        f"layer{2 * block_idx}.conv.weight"
                    ] = f"{res}x{res}/Conv0_up/weight"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.epilogue.bias"
            ...
            if res == self.init_res:
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx + 1}.conv.weight"
                ] = f"{res}x{res}/Conv/weight"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx + 1}.epilogue.bias"
                ] = f"{res}x{res}/Conv/bias"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx + 1}.epilogue.style_mod.dense.fc.weight"
                ] = f"{res}x{res}/Conv/StyleMod/weight"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx + 1}.epilogue.style_mod.dense.wscale.bias"
                ] = f"{res}x{res}/Conv/StyleMod/bias"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx + 1}.epilogue.apply_noise.weight"
            ...
            self.pth_to_tf_var_mapping[
                f"layer{2 * block_idx + 1}.epilogue.apply_noise.noise"
            ...
            self.pth_to_tf_var_mapping[
                f"output{block_idx}.conv.weight"
    ...
    def forward(
        self, w, generate_feature=False, generate_sw=False, generate_layer=False
    ):
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            if res_log2 + lod <= self.final_res_log2:
                ...
                if generate_feature:
                    feats.append(x)
                    layer_indice.append(2 * block_idx)
            ...
            if res_log2 + lod <= self.final_res_log2:
                ...
                if block_idx == 0:
                    x = self.__getattr__(f"layer{2 * block_idx}").mask_latent(
                        [w[:, 2 * block_idx] for w in ws], masks
                    )
                else:
                    x = self.__getattr__(f"layer{2 * block_idx}").mask_latent(
                        x, [w[:, 2 * block_idx] for w in ws], masks
                    )
                x = self.__getattr__(f"layer{2 * block_idx + 1}").mask_latent(
                    x, [w[:, 2 * block_idx + 1] for w in ws], masks
                )
                image = self.__getattr__(f"output{block_idx}")(x)
            else:
                image = self.upsample(image)
