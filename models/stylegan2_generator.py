...
class StyleGAN2Generator(BaseGenerator):
    ...
    def __init__(
        self, model_name, logger=None, truncation_psi=None, randomize_noise=False
    ):
        ...
        self.net.w_avg = self.net.truncation.w_avg
    ...
    def convert_tf_weights(self, test_num=10):
        ...
        tf_vars.update(dict(tf_net.components.mapping.__getstate__()["variables"]))
        tf_vars.update(dict(tf_net.components.synthesis.__getstate__()["variables"]))
    ...
    def _synthesize(
        ...
        if generate_style:
            ...
            style = self.net.synthesis.__getattr__(f"output{i // 2}").style(
                wps[:, i + 1, :]
            )
    ...
    def __call__(self, latent_codes, latent_space_type="z", labels=None):
        """Synthesizes images with given latent codes.
        ...
        image, feats, images = self.net.synthesis(
            wps, generate_feature=True, generate_imgsery=True
        )
