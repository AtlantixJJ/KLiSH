...
class SynthesisModule(nn.Module):
    ...
    def __init__(
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            ...
            if res == self.init_res:
                ...
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx - 1}.style.fc.weight"
                ] = f"{res}x{res}/Conv0_up/mod_weight"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx - 1}.style.bias"
                ] = f"{res}x{res}/Conv0_up/mod_bias"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx - 1}.noise_strength"
                ] = f"{res}x{res}/Conv0_up/noise_strength"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx - 1}.noise"
                ...
                if self.architecture_type == "resnet":
                    ...
                    self.pth_to_tf_var_mapping[
                        f"skip_layer{block_idx - 1}.weight"
            ...
            if res == self.init_res:
                ...
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.style.fc.weight"
                ] = f"{res}x{res}/Conv/mod_weight"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.style.bias"
                ] = f"{res}x{res}/Conv/mod_bias"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.noise_strength"
                ] = f"{res}x{res}/Conv/noise_strength"
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.noise"
                ] = f"noise{2 * block_idx}"
            else:
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.weight"
            ...
            if res_log2 == self.final_res_log2 or self.architecture_type == "skip":
                ...
                self.pth_to_tf_var_mapping[
                    f"output{block_idx}.style.fc.weight"
                ] = f"{res}x{res}/ToRGB/mod_weight"
                self.pth_to_tf_var_mapping[
                    f"output{block_idx}.style.bias"
    ...
    def forward(self, w, generate_feature=False, generate_layer=False):
        ...
        if self.architecture_type == "origin":
            for layer_idx in range(self.num_layers - 1):
                ...
                feats.append(x)
                layer_indice.append(layer_idx + 1)
        ...
        if self.architecture_type == "origin":
            for layer_idx in range(self.num_layers - 1):
                x = self.__getattr__(f"layer{layer_idx}").mask_latent(
                    x, [w[:, layer_idx] for w in ws], masks
                )
                feats.append(x)
            image = self.__getattr__(f"output{layer_idx // 2}")(x, w[:, layer_idx + 1])
        elif self.architecture_type == "skip":
            ...
            for layer_idx in range(1, self.num_layers - 1, 2):
                ...
                x = self.__getattr__(f"layer{layer_idx + 1}").mask_latent(
                    x, [w[:, layer_idx + 1] for w in ws], masks
                )
                x = (x + residual) / np.sqrt(2.0)
                feats.append(x)
            image = self.__getattr__(f"output{layer_idx // 2 + 1}")(
...
class InputBlock(nn.Module):
    ...
    def mask_latent(self, ws, masks):
        x = sum(
            [
                mask * self.const.repeat(w.shape[0], 1, 1, 1)
