...
class PGGANGeneratorNet(nn.Module):
    ...
    def __init__(
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            ...
            if res == self.init_res:
                ...
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx}.wscale.bias"
            ...
            self.add_module(
                ...
                ConvBlock(in_channels=self.get_nf(res), out_channels=self.get_nf(res)),
            )
            if res == self.init_res:
                self.pth_to_tf_var_mapping[
                ...
                self.pth_to_tf_var_mapping[
                    f"layer{2 * block_idx + 1}.wscale.bias"
            ...
            self.pth_to_tf_var_mapping[
                f"output{block_idx}.wscale.bias"
    ...
    def forward(self, z, l=None, generate_feature=False, generate_layer=False):
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            if res_log2 + lod <= self.final_res_log2:
                ...
                if generate_feature:
                    feats.append(x)
                    layer_indice.append(block_idx * 2)
                x = self.__getattr__(f"layer{2 * block_idx + 1}")(x)
                if generate_feature:
                    feats.append(x)
                    layer_indice.append(block_idx * 2 + 1)
