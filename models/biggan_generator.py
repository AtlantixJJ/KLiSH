...
class Generator(nn.Module):
  ...
  def forward(self, cond_vector, truncation, generate_feature=False):
    #cond_vector = cond_vector.permute(1, 0, 2)
    L, N, C = cond_vector.shape
    z = self.gen_z(cond_vector[0]) # Use layer 0 to get input z

    # We use this conversion step to be able to use TF weights:
    # TF convention on shape is [batch, height, width, channels]
    # PT convention on shape is [batch, channels, height, width]
    z = z.view(N, 4, 4, 16 * self.config.channel_width)
    ...
    for layer in self.layers:
      if isinstance(layer, GenBlock):
        z = layer(z, cond_vector[latent_indice], truncation)
        latent_indice += 1
      else:
        z = layer(z)
      if generate_feature:
...
class Latents(torch.nn.Module):
  def __init__(self, num_latents=15, num_classes = 1000, z_dim=128,
    max_classes=None, class_temperature=2.0):
    super().__init__()
...
class BigGANWrapper(object):
  def __init__(self, resolution):
    self.net = BigGAN.from_pretrained(f"biggan-deep-{resolution}",
      cache_dir="models/pretrain/pytorch/biggan")
...
class BigGAN(nn.Module):
  ...
  def sample_latent(self, N, z_dim=128, n_class=1000):
    n_layer = len(self.generator.layers)
    device = self.embeddings.weight.device
    z = torch.zeros(n_layer, N, z_dim).normal_(std=1).to(device)
    #c = torch.sigmoid(torch.zeros(n_layer, N, n_class).normal_(
    #  mean=-3.9, std=.3).to(device))
    if self.class_cond < 0:
      ind = torch.randint(0, n_class, (1, N, 1))
      c = torch.zeros(1, N, n_class).scatter_(2, ind, 1)
      c = c.repeat(n_layer, 1, 1).to(device)
    else:
      c = torch.zeros(n_layer, N, n_class).to(device)
      c[..., self.class_cond] = 1
    return z, c

  def forward(self, latent, truncation=0.5,
    generate_feature=False):
    assert 0 < truncation <= 1
    z, c = latent
    L, N, n_class = c.shape
    embed = self.embeddings(c.view(L * N, n_class)).view(L, N, -1)
    cond_vector = torch.cat((z, embed), dim=2)

    res = self.generator(cond_vector, truncation, generate_feature)
    return res