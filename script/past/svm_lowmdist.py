...
def calc_metric_worker(feat, svm_label, w):
  ...
  with torch.no_grad():
    for i in range(N):
      ...
      # how much positive samples fall into margins
      #ld_area = ((seg >= -1) & (seg <= 1))
...
def loss_func_batched_l2svc(feat, w, svm_label):
  ...
  for i in range(N):
    ...
    svm_loss = torch.square((1 - svm_label[st:ed] * seg).clamp(min=0)).mean() / N * 10
...
def train_svm_l2reg_l2svc(mfeat, w, optim, msvm_label, mlabel=None):
  ...
  #reg_loss = 1e-3 * w.abs().sum(1).mean()
  reg_loss.backward()
...
if __name__ == "__main__":
  ...
  parser.add_argument("--name", default="svm_lowmdist")
  # architecture
  ...
  for i in tqdm(range(200)):
    ...
    record.add("Max_p E(p's dist2 to margin | p)", dm[p])
    record.add("1/M Sum_p E(p's dist2 to margin | p)", dm.mean())
    ...
    def test_closure():
      return train_svm_l2reg_l2svc(mfeat, test_w, test_optim, msvm_label, mlabel)
    for j in range(args.svm_iter):
