...
from lib.op import bu, torch2numpy, torch2image, GeneralThread
...
class PlannarVideoRecorder:
    """Visualize a clustering (on 2D data only) process to a video."""

    def __init__(self, xs, prefix, resolution=1024):
        self.frames = []
        self.xs = xs
        self.xs_np = torch2numpy(xs)
        self.prefix = prefix
        self.resolution = resolution
        x_min, y_min = xs.min(0).values.cpu()
        x_max, y_max = xs.max(0).values.cpu()
    ...
    def __call__(self, alg):
        ...
        alg.fitted = True
        with torch.no_grad():
            n_cluster = list(alg.merge_record.keys())[-1]
            gt_y = torch2numpy(alg.my[0])
            c = high_contrast_arr[gt_y] / 255.0
            smap = alg.predict(self.xy.view(-1, 2), n_cluster, ret_raw=True)  # (N, K)
            smap = smap.unsqueeze(2) - smap.unsqueeze(1)  # (N, K, K)
            for i in range(smap.shape[1]):
                smap[:, i, i] = 1e10
            smap = smap.min(2).values
            smap = smap.view(self.resolution, self.resolution, -1)
            smap = smap.max(2).values
            fig = plt.figure(figsize=(10, 10))
            plt.contour(
                self.xy[..., 0],
                self.xy[..., 1],
                smap,
                levels=[1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 1, 2, 10],
                alpha=0.5,
            )
            plt.scatter(self.xs_np[:, 0], self.xs_np[:, 1], s=2, c=c)
            plt.xlim([self.x_min, self.x_max])
            plt.ylim([self.y_min, self.y_max])
            mld = alg.merge_record[n_cluster]["Merge Metric"]
            plt.title(f"#clusters={n_cluster}; MLD={mld:03f}", fontsize=20)
            plt.tight_layout()
    ...
    def clean_sync(self):
        ...
        VideoWriter.gen(f"{self.prefix}_decision.mp4", self.frames)
