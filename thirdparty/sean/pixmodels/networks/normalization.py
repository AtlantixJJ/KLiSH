...
from pixmodels.networks.sync_batchnorm import SynchronizedBatchNorm2d
...
class ACE(nn.Module):
                    ...
                    for j in range(segmap.shape[1]):
                        ...
                        if component_mask_area > 0:
                            if obj_dic is None:
                                ...
                                middle_mu = F.relu(self.fc_mu[j](style_code_tmp))
                ...
                for i in range(b_size):
                    for j in range(segmap.shape[1]):
                        ...
                        if component_mask_area > 0:
                            ...
                            middle_mu = F.relu(self.fc_mu[j](style_codes[i][j]))
                            component_mu = middle_mu.reshape(self.style_length, 1).expand(self.style_length, component_mask_area)
    ...
    def create_gamma_beta_fc_layers(self):
        ...
        for i in range(self.label_nc):
            self.fc_mu.append(nn.Linear(style_length, style_length))
