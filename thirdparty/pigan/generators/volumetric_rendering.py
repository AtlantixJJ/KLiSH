...
def feature_integration(sigmas, feature, z_vals, device, noise_std=0.5, last_back=False, white_back=False, clamp_mode=None, fill_mode=None):
    """Performs NeRF volumetric rendering."""
    ...
    feature_final = torch.sum(weights.to(feature) * feature, -2)

    if white_back:
        feature_final = feature_final + 1 - weights_sum

    if fill_mode == 'debug':
        feature_final[weights_sum.squeeze(-1) < 0.9] = torch.tensor([1., 0, 0], device=feature_final.device)
    elif fill_mode == 'weight':
        feature_final = weights_sum.expand_as(feature_final)
