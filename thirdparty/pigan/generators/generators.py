...
class ImplicitGenerator3d(nn.Module):
    ...
    def staged_forward_feature(self, z, img_size, fov, ray_start, ray_end, num_steps, h_stddev, v_stddev, h_mean, v_mean, psi=1, lock_view_dependence=False, max_batch_size=50000, depth_map=False, near_clip=0, far_clip=2, sample_dist=None, hierarchical_sample=False, **kwargs):
        """
        ...
        with torch.no_grad():
            ...
            truncated_frequencies = self.avg_frequencies + psi * (raw_frequencies - self.avg_frequesshncies)
            ...
            for b in range(batch_size):
                ...
                while head < transformed_points.shape[1]:
                    ...
                    output, feature = self.siren.forward_with_frequencies_phase_shifts_feature(transformed_points[b:b+1, head:tail], truncated_frequencies[b:b+1], truncated_phase_shifts[b:b+1], ray_directions=transformed_ray_directions_expanded[b:b+1, head:tail])
                    coarse_output[b:b+1, head:tail] = output
                    if coarse_feature is None:
                        #coarse_feature = torch.zeros((batch_size, transformed_points.shape[1], feature.shape[-1]))
                        coarse_feature = torch.zeros((batch_size, transformed_points.shape[1], feature.shape[-1]), device=self.device)
                    coarse_feature[b:b+1, head:tail].copy_(feature)
                    head += max_batch_size

            coarse_output = coarse_output.reshape(batch_size, img_size * img_size, num_steps, 4)
            coarse_feature = coarse_feature.reshape(batch_size, img_size * img_size, num_steps, -1)
            ...
            if hierarchical_sample:
                ...
                for b in range(batch_size):
                    ...
                    while head < fine_points.shape[1]:
                        tail = head + max_batch_size
                        output, feature = self.siren.forward_with_frequencies_phase_shifts_feature(fine_points[b:b+1, head:tail], truncated_frequencies[b:b+1], truncated_phase_shifts[b:b+1], ray_directions=transformed_ray_directions_expanded[b:b+1, head:tail])
                        fine_output[b:b+1, head:tail] = output
                        if fine_feature is None:
                            #fine_feature = torch.zeros((batch_size, fine_points.shape[1], feature.shape[-1]))
                            fine_feature = torch.zeros((batch_size, fine_points.shape[1], feature.shape[-1]), device=self.device)
                        fine_feature[b:b+1, head:tail].copy_(feature)
                        head += max_batch_size

                fine_output = fine_output.reshape(batch_size, img_size * img_size, num_steps, 4)
                fine_feature = fine_feature.reshape(batch_size, img_size * img_size, num_steps, -1)

                all_outputs = torch.cat([fine_output, coarse_output], dim = -2)
                all_features = torch.cat([fine_feature, coarse_feature], dim = -2)
                all_z_vals = torch.cat([fine_z_vals, z_vals], dim = -2)
                _, indices = torch.sort(all_z_vals, dim=-2)
                all_z_vals = torch.gather(all_z_vals, -2, indices)
                all_outputs = torch.gather(all_outputs, -2, indices.expand(-1, -1, -1, 4))
                #all_features = torch.gather(all_features, -2, indices.expand(-1, -1, -1, fine_feature.shape[-1]).cpu())
                all_features = torch.gather(all_features, -2, indices.expand(-1, -1, -1, fine_feature.shape[-1]))
            else:
                all_outputs = coarse_output
                all_features = coarse_feature
                all_z_vals = z_vals
            ...
            features = feature_integration(all_outputs[..., 3:], all_features, all_z_vals, device=self.device, white_back=kwargs.get('white_back', False), clamp_mode = kwargs['clamp_mode'], last_back=kwargs.get('last_back', False), fill_mode=kwargs.get('fill_mode', None), noise_std=kwargs['nerf_noise'])

            pixels = pixels.reshape((batch_size, img_size, img_size, 3))
            pixels = pixels.permute(0, 3, 1, 2).contiguous().cpu() * 2 - 1

        return features, pixels, depth_map
