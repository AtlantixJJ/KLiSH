...
class ScenePredictor(BasePredictor):
  ...
  def build(self):
    self.net = resnet18(num_classes=self.n_class)
  ...
  def load(self):
    ...
    assert len(self.category_name_to_idx) == self.n_class
    assert len(self.category_idx_to_name) == self.n_class
  ...
  def _predict(self, images):
    ...
    assert category_scores.shape == (images.shape[0], self.n_class)
  ...
  def save(self, predictions, save_dir):
    ...
    assert categories.ndim == 2 and categories.shape[1] == self.n_class
    detailed_categories = {
        'score': categories,
